...
export function AddImageScreen({ navigation, route }) {
        ...
        <Grid>
          ...
          <Button block
            onPress={() => {
              ...
              RNFS.readDir(global.APP_FILES_PATH)
              .then((result) => {
              ...
              .then((statResult) => {
                ...
                if (statResult[0].isFile()) {
                  ...
                  setCurrentImagePath(global.APP_FILES_PATH + fileName);
          ...
          <Button block
            onPress={() => {
              // get a list of files and directories in the main bundle
              // RNFS.readDir(RNFS.MainBundlePath) // On Android, use "RNFS.DocumentDirectoryPath" (MainBundlePath is not defined)
              // RNFS.readFile(global.APP_FILES_PATH + '/test.txt', 'ascii')
              const fileName = '/2020-05-04T13-54-26.822Z.jpg';
              RNFS.readFile(global.APP_FILES_PATH + fileName, 'base64')
              .then((result) => {
          ...
          <Button block
            onPress={() => {
              ...
              RNFS.mkdir(global.APP_FILES_PATH, {NSURLIsExcludedFromBackupKey: true});
              /* Write file */
              var path = global.APP_FILES_PATH + '/test.txt';
          ...
          <Button block
            onPress={() => {
              const options = {
                ...
                // maxWidth: global.IMAGE_WIDTH,
                // maxHeight: global.IMAGE_HEIGHT,
              };
              ImagePicker.showImagePicker(options, (response) => {
                ...
                if (response.didCancel) {
                  ...
                  ImageResizer.createResizedImage(response.uri, global.IMAGE_WIDTH, global.IMAGE_HEIGHT, 'JPEG', 100, 0, undefined, false)
                  .then((response) => {
                    console.log('Resized image in cache', response.uri);

                    /* With image cropper */
                    navigation.navigate('AmazingCropper', {imageUri: response.uri, hideTabBar: true});

                    // /* Without image cropper */
                    // /* copy image into the APP_FILES_PATH */
                    // const imagePath = `${global.APP_FILES_PATH}/${new Date().toISOString()}.jpg`.replace(/:/g, '-');
                    // if(Platform.OS === 'ios') {
